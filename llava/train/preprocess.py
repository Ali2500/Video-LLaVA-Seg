...
from llava.constants import (DEFAULT_IM_END_TOKEN, DEFAULT_IM_START_TOKEN,
                             ...
                             IMAGE_TOKEN_INDEX, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, 
                             DEFAULT_VID_END_TOKEN, DEFAULT_SF_VID_SEPARATOR_TOKEN, DEFAULT_VID_SEG_TOKEN)
    ...
    for source in sources:
        for sentence in source:
            ...
            if DEFAULT_VIDEO_TOKEN in sentence['value']:
                assert sentence['value'].count(DEFAULT_VIDEO_TOKEN) == 1  # only allow 1 video
                if data_args.num_slow_frames == data_args.num_frames:
                    num_slow_frames = data_args.num_frames
                    num_fast_frames = 0
                    sf_separator = ""
                else:
                    num_slow_frames = data_args.num_slow_frames
                    num_fast_frames = data_args.num_frames
                    sf_separator = DEFAULT_SF_VID_SEPARATOR_TOKEN if data_args.mm_use_sf_vid_separator_token else ""

                # vid_replace_token = DEFAULT_IMAGE_TOKEN * num_frames_per_video
                vid_replace_token = (DEFAULT_IMAGE_TOKEN * num_slow_frames) + sf_separator + (DEFAULT_IMAGE_TOKEN * num_fast_frames)
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        if DEFAULT_VID_SEG_TOKEN in source[1]['value']:
            raise ValueError(f"Referral type prompt is not compatible with 'plain' preprocessing")

        num_frames = source[0]['value'].count(DEFAULT_IMAGE_TOKEN)
        source[0]['value'] = DEFAULT_IMAGE_TOKEN * num_frames
